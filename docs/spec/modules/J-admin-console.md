...
| A-10 管理员管理 | 管理员列表（角色/状态 Tag、最后超管保护警示条）、邀请管理员抽屉（邮箱/角色/角色说明/SMTP 引导） | J9 |
| A-11 用户查询 | 脱敏 openid 列表、隐私提示条、筛选、用户画像抽屉（2×2 统计卡、留痕说明） | J10 |
| A-12 审计日志 | 操作者/动作/日期筛选、动作分类 Tag、密钥仅记 last4、IP 列 | J11 |

> 更新（2026-09-12）：A-10~A-12 已生成（复用 A-06 框架壳 + 「权限与审计」侧栏分组；audit 清零：CLIP 0 / NOFILL 0 / 字阶合规 / 仅 lucide；画布逐板目检通过）。注：Pencil Export 管线对新节点输出过期快照，`docs/ui/exports/` 下 a10~a12 PNG 暂缺待补导；另 A-08/A-09 存量 11 处 fontSize=10 不在字阶白名单，属前代遗留，待设计复审。
